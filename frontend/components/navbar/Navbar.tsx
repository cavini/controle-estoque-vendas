...
const Navbar = () => {
  ...
  const links = [{ url: "/login", title: "Entrar" },
  ...
  return (
    <nav className={styles.nav}>
      ...
      <ul className={`${styles.navLinks} ${active ? styles.navActive : ""}`}>
        {links.map((link, index) => (
          <a
            className={styles.test}
            style={{
              animation: `${styles.navLinkFade} 0.5s ease forwards ${ index / 7}s`
            }}
          >
            {link.title}
          </a>
